...
// cache para no repetir mensajes casi iguales
const lastMessages = [];
const LAST_N = 12;
...
async function acceptCookiesAnywhere(p) {
  ...
  for (const t of texts) try {
    ...
    if (await b.count() && await b.first().isVisible({ timeout: 300 })) {
      await b.first().click({ timeout: 1500 }); await p.waitForTimeout(200);
    }
  } catch {}
}
async function nukeOverlays(p) {
  try {
    await p.evaluate(() => {
      ...
      kill.forEach(sel => document.querySelectorAll(sel).forEach(el => { try{ el.style.display="none"; el.remove?.(); }catch{} }));
      const BIG = Array.from(document.querySelectorAll("body *")).filter(el=>{
        const s=getComputedStyle(el); const pos=s.position; const zi=parseInt(s.zIndex||"0",10);
...
async function waitUntilEnabled(locator, timeout=6000) {
  const t0 = Date.now();
  while (Date.now()-t0 < timeout) { try { if (!(await locator.isDisabled())) return true; } catch {} await page.waitForTimeout(120); }
  return false;
}
...
async function generateReply(chat, perfil) {
  const body = {
    model: MODEL, temperature: 0.95, // más variación
    messages: [
  ...
  // si aún parecido, pide reparación
...
/* ---- encontrar input y teclear COMO HUMANO ---- */
const SELECTORS_INPUT = SELECTORS.inputCandidates;

async function findInputHandle(){
  await nukeOverlays(page);
  for (const sel of SELECTORS_INPUT) {
...
async function typeLikeHuman(text){
  // no pegar: siempre teclado
  // pequeñas pausas y micro-correcciones
  for (let i=0;i<text.length;i++){
    const ch = text[i];
    // 1 de cada ~30 chars, pausa larga
    if (i>0 && i%rand(28,34)===0) await page.waitForTimeout(rand(180,420));
    // 1 de cada ~22 chars, simula errata y corrige
    if (i>3 && i%rand(20,24)===0) {
...
async function sendMessage(text){
  ...
  // LIMPIAR sin "fill" (para no disparar detección)
  ...
  // triple click también ayuda en contenteditable
