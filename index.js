...
async function isLoginForm() {
  try {
    const e = page.locator(emailSel);
    const p = page.locator(passSel);
    return (await e.count()) && (await p.count());
  } catch { return false; }
}
...
async function performLogin() {
  ...
  await e.waitFor({ state: "visible", timeout: 5000 }).catch(()=>{});
  await p.waitFor({ state: "visible", timeout: 5000 }).catch(()=>{});
  // escribir despacio y validar valores
  ...
  await waitUntilEnabled(btn, 5000); // si se habilita, clic
  try { await btn.click({ timeout: 2000 }); } catch { try { await p.press("Enter"); } catch {} }

  // esperar salida del login (desaparece el input password)
  await page.waitForFunction(() => !document.querySelector('input[type="password"], input[placeholder*="password" i]'), { timeout: 8000 }).catch(()=>{});
}
...
async function isQueueVisible() {
  ...
  const start = await page.getByRole("button",{ name:/start chatting|start chat|start/i }).first().isVisible().catch(()=>false);
  return wait || start;
}
...
async function generateReply(chat, perfil) {
  ...
  for (const w of BANNED) if (lower.includes(w)) {
    text = await generateRepair(text); break;
  }
...
async function loop() {
  ...
  while (true) {
    try {
      ...
      // si vuelve a salir login, vuelve a loguear y no intentes escribir
